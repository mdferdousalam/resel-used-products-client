...
const AppleCategory = () => {
    UseTitle('Apple Category')
    const { loading, setLoading, user } = useContext(AuthContext)

    const [apple, setApple] = useState(null)

    const url = `https://b612-used-products-resale-server-side.vercel.app/products/apple`
    useEffect(() => {
    ...
    return (
        <div className='mt-10 grid gap-10 grid-cols-1 md:grid-cols-2 lg:grid-cols-3'>
            ...
            {
                apple?.map(phone => <div className="card text-primary w-96 bg-base-100 shadow-xl">
